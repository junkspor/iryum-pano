...
# --- 5. FİYAT GİRİŞ PANELİ (HATAYA KARŞI DÜMDÜZ EDİLDİ) ---
...
y_ref = frm.number_input("O ANKİ HAS FİYATI:", value=st.session_state.ref_has, step=10.0)

frm.markdown("### 2. Tek Fiyatlı Ürünler")
c1, c2 = frm.columns(2)
y_24 = c1.number_input("24 Ayar (Sadece Satış)", value=st.session_state.g_24, step=10.0)
y_22_s = c1.number_input("22 Ayar (SATIŞ)", value=st.session_state.g_22_s, step=10.0)
y_14 = c2.number_input("14 Ayar (Sadece Satış)", value=st.session_state.g_14, step=10.0)
y_22_a = c2.number_input("22 Ayar (ALIŞ)", value=st.session_state.g_22_a, step=10.0)
frm.markdown("### 3. Çift Fiyatlı Ürünler (Alış - Satış)")
c3, c4 = frm.columns(2)
y_besli_a = c3.number_input("Beşli (Alış)", value=st.session_state.g_besli_a, step=10.0)
y_tam_a = c3.number_input("Tam (Alış)", value=st.session_state.g_tam_a, step=10.0)
y_yarim_a = c3.number_input("Yarım (Alış)", value=st.session_state.g_yarim_a, step=10.0)
y_ceyrek_a = c3.number_input("Çeyrek (Alış)", value=st.session_state.g_ceyrek_a, step=10.0)
y_gram_a = c3.number_input("Gram (Alış)", value=st.session_state.g_gram_a, step=10.0)

y_besli_s = c4.number_input("Beşli (Satış)", value=st.session_state.g_besli_s, step=10.0)
y_tam_s = c4.number_input("Tam (Satış)", value=st.session_state.g_tam_s, step=10.0)
y_yarim_s = c4.number_input("Yarım (Satış)", value=st.session_state.g_yarim_s, step=10.0)
y_ceyrek_s = c4.number_input("Çeyrek (Satış)", value=st.session_state.g_ceyrek_s, step=10.0)
y_gram_s = c4.number_input("Gram (Satış)", value=st.session_state.g_gram_s, step=10.0)
...
submit_button = frm.form_submit_button(label="✅ RAKAMLARI SİSTEME İŞLE VE GÜNCELLE")
