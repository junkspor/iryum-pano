...
def oanda_ons_satis() -> Optional[float]:
    """OANDA'dan XAU_USD 'ask' (satış) fiyatını çeker. Secrets yoksa None döner."""
    try:
        token = st.secrets.get("OANDA_TOKEN", None)
        account_id = st.secrets.get("OANDA_ACCOUNT_ID", None)
        ...
        r = requests.get(url, headers=headers, params={"instruments": "XAU_USD"}, timeout=15)
        if r.status_code != 200:
            return None
        data = r.json()
...
# 1) ONS: OANDA (satış/ask). OANDA yoksa Harem'den gelen ONS'a düş.
ons = oanda_ons_satis() or f.get("ons_satis")
if ons is None:
    st.warning("ONS fiyatı alınamadı (OANDA/Harem). Ekranda 0 gösterilecek.")
    ons = 0.0

# 2) USD ve HAS: Harem'le aynı kalsın (satış)
dolar = f.get("usdtry_satis")
canli_teorik_has = f.get("has_satis")

# 3) USDKG: manuel gireceğiz (session_state'ten) — burada çekmiyoruz
altinkg_usd = None

if (dolar is None) or (canli_teorik_has is None):
    st.error(
        "Harem Altın'dan USD veya HAS çekilemedi. "
        "(Geçici engel/endpoint değişimi olabilir.)"
    )
    st.stop()

if ons is None:
    st.warning("ONS (XAUUSD) şu an çekilemedi. OANDA secrets'lerini kontrol edin.")
    ons = 0.0
...
# Manuel USDKG (Satış) değeri (varsa) — ekranda göstermek için
_alt = float(st.session_state.get('altinkg_usd_satis', 0.0) or 0.0)
altinkg_usd = _alt if _alt > 0 else None
...
# (Opsiyonel) USDKG satış değerini manuel girebilirsin (Harem ekranındaki USD KG Satış)
...
if buton:
    ...
    st.session_state.altinkg_usd_satis = y_altinkg
